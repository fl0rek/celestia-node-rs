...
//const LRA_WINDOW_SIZE: usize = 16 * 1024;

// Least Recently Added Store
#[derive(Debug)]
pub struct FifoStore {
    headers: DashMap<Hash, ExtendedHeader>,
    height_to_hash: DashMap<u64, Hash>,
    head_height: AtomicU64,
    header_window_size: u64,
}

impl FifoStore {
    pub fn new(header_window_size: u64) -> Self {
        FifoStore {
            headers: DashMap::new(),
    ...
    fn remove_tail(&self, height: u64) {
        info!("removing tail header from the store {height}");
        let Entry::Occupied(tail_hash) = self.height_to_hash.entry(height) else {
            warn!("trying to remove unknown height {height}");
            return;
        };

        if self.headers.remove(tail_hash.get()).is_none() {
            warn!("trying to remove unknown hash {}", tail_hash.get());
        }
    ...
    pub fn append_single_unchecked(&self, header: ExtendedHeader) -> Result<()> {
        ...
        if let Some(tail_height) = height.checked_sub(self.header_window_size) {
            self.remove_tail(tail_height);
        }
...
pub mod tests {
    ...
    #[test]
    fn lra_test() {
        let s = FifoStore::new(4);
        let mut gen = ExtendedHeaderGenerator::new();

        assert!(!s.contains_height(1));
        s.append_single_unchecked(gen.next()).unwrap();
        assert!(s.contains_height(1));
        s.append_single_unchecked(gen.next()).unwrap();
        s.append_single_unchecked(gen.next()).unwrap();
        s.append_single_unchecked(gen.next()).unwrap();
        assert!(s.contains_height(1));
        assert!(s.contains_height(2));
        assert!(s.contains_height(3));
        assert!(s.contains_height(4));
        s.append_single_unchecked(gen.next()).unwrap();
        assert!(!s.contains_height(1));
        assert!(s.contains_height(5));
        s.append_single_unchecked(gen.next()).unwrap();
        assert!(!s.contains_height(2));
        assert!(s.contains_height(6));
    }

    pub fn gen_filled_store(amount: u64) -> (FifoStore, ExtendedHeaderGenerator) {
        let s = FifoStore::new(1024);
        let mut gen = ExtendedHeaderGenerator::new();
